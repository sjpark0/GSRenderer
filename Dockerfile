...
RUN pip install --no-build-isolation mmcv==1.6.0
RUN pip install opencv-python pyyaml plyfile tqdm termcolor kornia imgaug lpips tensorboardX ipdb scikit-image imageio imageio[ffmpeg] imageio[pyav] argparse pytorch_msssim open3d
COPY submodules /opt/lib
WORKDIR /opt

ENV TORCH_CUDA_ARCH_LIST="7.0;7.5;8.0;8.6;8.9;9.0+PTX"

RUN pip install --no-build-isolation lib/depth-diff-gaussian-rasterization
RUN pip install --no-build-isolation lib/simple-knn
RUN python -c "import torchvision;torchvision.models.vgg16(pretrained=True)"
